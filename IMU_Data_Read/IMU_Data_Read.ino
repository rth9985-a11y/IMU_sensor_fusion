...
int8_t bmi2_i2c_read(uint8_t reg, uint8_t *data, uint32_t len, void *intf_ptr) {
  ...
  if (Wire2.endTransmission(false) != 0) return BMI2_E_COM_FAIL;

  /*Set length of byte so send from address*/
  Wire2.requestFrom((uint8_t)BMI270_I2C_ADDR, (uint8_t)len);
  /*Loop over data stream in length of bytes ????????????? IS THIS A BUFFER*/
...
void loop() {
  if (bmi2_get_sensor_data(&sensor_data, &bmi) == BMI2_OK) {
    Serial.print(sensor_data.acc.x); Serial.print(",");
    Serial.print(sensor_data.acc.y); Serial.print(",");
    Serial.print(sensor_data.acc.z); Serial.print(",");
    Serial.print(sensor_data.gyr.x); Serial.print(",");
    Serial.print(sensor_data.gyr.y); Serial.print(",");
